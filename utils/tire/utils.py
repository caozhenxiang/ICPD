...
def set_windowsize_and_threshold(dataset):
    if (
        (("AR" in dataset) and not ("VAR" in dataset))
        or (("ar" in dataset) and not ("var" in dataset))
        or ("MC" in dataset)
    ):
        windowsize = 40
        threshold = 45
        ...
        for keyword in ["mean", "MEAN", "var", "VAR", "gauss", "GAUSS", "ar"]:
            if keyword in dataset:
                windowsize = 20
                threshold = 20
        for keyword in ["run", "cycle", "walk", "stand"]:
            if keyword in dataset:
                windowsize = 300
                threshold = 200
...
def clustering(
    ...
    for i in np.arange(k):
        ...
        for s in np.arange(num_array.size):
            dis_matrix[s, merge_idx] = (na / (na + nb)) * dis_matrix[
                s, merge_idx
            ] + (nb / (na + nb)) * dis_matrix[s, merge_idx + 1]
            dis_matrix[merge_idx, s] = (na / (na + nb)) * dis_matrix[
                merge_idx, s
            ] + (nb / (na + nb)) * dis_matrix[merge_idx + 1, s]
...
def clustering2(
    ...
    dissimilarities = smoothened_dissimilarity_measures(
        encoded_windows, encoded_windows_fft, window_size
    )
    dissimilarities = np.concatenate(
        (np.zeros((window_size,)), dissimilarities, np.zeros((window_size,)))
    )
    ...
    while nr_classes > k:
        ...
        new_dispersion = (
            (na * dispersion_array[merge_idx])
            + (nb * dispersion_array[merge_idx + 1])
            + (na * nb * dis_array[merge_idx])
        ) / (na + nb + na * nb)
