""" Define functions to create the triplet loss with online triplet mining.
    ...
    Github repository: https://github.com/omoindrot/tensorflow-triplet-loss
    Blog: https://omoindrot.github.io/triplet-loss
    Modified by Zhenxiang Cao on 2022.01.08 """
...
def _get_anchor_negative_triplet_mask(labels):
    ...
    labels_equal_pre = tf.equal(
        tf.expand_dims(labels, 0), tf.expand_dims(labels - 1, 1)
    )
    labels_equal_next = tf.equal(
        tf.expand_dims(labels, 0), tf.expand_dims(labels + 1, 1)
    )
    mask = tf.logical_or(labels_equal_pre, labels_equal_next)
...
def _get_triplet_mask(labels):
    ...
    label_not_equal = tf.logical_or(labels_equal_pre, labels_equal_next)

    i_not_equal_k = tf.expand_dims(label_not_equal, 1)

    valid_labels = tf.logical_and(i_equal_j, i_not_equal_k)
...
def batch_hard_triplet_loss(labels, embeddings, margin, squared=False):
    ...
    """
    # Get the pairwise distance matrix
    embeddings_positive = embeddings[:, 0, :]
    embeddings = embeddings[:, 1, :]
    pairwise_dist = _pairwise_distances(embeddings, squared=squared)
    positive_dist = tf.reduce_sum(
        tf.square(embeddings - embeddings_positive), axis=1, keepdims=True
    )
    if not squared:
        positive_dist = tf.sqrt(positive_dist)
    tf.summary.scalar("positive_dist", tf.reduce_mean(positive_dist))
    """
    ...
    tf.summary.scalar(
        "hardest_negative_dist", tf.reduce_mean(hardest_negative_dist)
    )

    # Combine biggest d(a, p) and smallest d(a, n) into final triplet loss
    triplet_loss = tf.maximum(
        positive_dist - hardest_negative_dist + margin, 0.0
    )
